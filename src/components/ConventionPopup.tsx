...
export default function ConventionPopup() {
    ...
    return (
        <>
            <button className="bg-hw-white fixed top-1/2 left-0 z-10 -translate-y-1/2 rounded-r-3xl p-5 shadow-lg duration-200 hover:px-7" type="button" onClick={() => setOpen('true')}>
                ...
                {open === 'true' && (
                    <ModalBackdrop onClose={() => setOpen('false')}>
                        <m.div
                            ...
                            className={clsx(
                                'bg-hw-white absolute top-1/2 left-1/2 z-50 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-3 overflow-hidden p-10 text-center',
                            )}
                        >
                            ...
                            <button onClick={() => setOpen('false')} type="button">
                                <Close className="text-hw-black absolute top-5 right-5 duration-200 hover:opacity-75" />
                            </button>
                            <span className="mt-10 text-xl">Important Links:</span>
                            <div className="text-hw-black flex flex-row gap-5 text-xl underline">
