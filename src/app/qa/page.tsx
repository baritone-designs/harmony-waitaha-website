...
const SocialLink: FC<SocialLinkProps> = ({ href, icon: Icon, className }) => (
    <a
        ...
        target="_blank"
        rel="noreferrer"
        className="duration-200 hover:text-qa-blue hover:drop-shadow-qa-glow-intense"
    >
...
export default async function QAHome() {
    ...
    } = await prisma.chorus.findFirstOrThrow({
        where: {
            id: ChorusId.Qa,
        },
        select: {
    ...
    const people = (
        await prisma.personChorus.findMany({
            where: {
                    ...
                    <div className="flex w-screen justify-center">
                        <div className="w-full max-w-screen-2xl px-5 lg:px-20">
                            <div className="flex flex-col text-center text-5xl font-medium lg:text-left lg:text-8xl">
                                <span>
                                    <span className="font-semibold text-qa-blue drop-shadow-qa-glow-light">
                                        Q
                                    </span>
                                    <span>uantum</span>
                                </span>
                                        ...
                                        {Object.entries(socials).map(
                                            ([key, value]) => value && (
                                                <SocialLink
                                                    key={key}
                                                    icon={
                                                        SOCIALS_ICONS[key as keyof typeof SOCIALS_ICONS]
                                                    }
                                                    href={
                                                        SOCIALS_PREFIX[
                                key as keyof typeof SOCIALS_PREFIX
                                                        ] + value
                                                    }
                ...
                <div className="w-full max-w-screen-2xl px-5 lg:px-20">
                    <section id="about" className="mb-10 space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2 lg:gap-20">
                            <p className="z-10">{aboutParagraph}</p>
                            <div>
                                <MediaCarousel
                                    className="z-10 aspect-video w-full rounded-3xl"
                                    mediaUrls={carouselMediaUrls}
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex w-full justify-center">
                            <div className="grid grid-cols-2 items-center justify-between gap-3 lg:flex lg:flex-row lg:gap-14">
                                ...
                                {people.length === 0 && (
                                    <span>
                                        We have team members we would like to show here, but they
                                        are a bit shy, why don't you come along to one of our
                                        rehearsals to meet them for real!
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    <section
                        id="footer"
                        className="mb-5 flex flex-row items-end justify-between lg:mb-10"
                    >
                        <div
                            className={clsx(
                                Object.values(socials).filter((x) => !!x).length === 0
                  && 'invisible',
                            )}
                        >
                            <span className="text-4xl font-semibold text-qa-blue">
                                ...
                                {Object.entries(socials).map(
                                    ([key, value]) => value && (
                                        <SocialLink
                                            ...
                                            href={
                                                SOCIALS_PREFIX[key as keyof typeof SOCIALS_PREFIX]
                          + value
                                            }
                        ...
                        <a
                            className="hidden h-full flex-col items-center text-qa-blue duration-200 hover:drop-shadow-qa-glow-intense lg:flex"
                            href="/"
                            target="_blank"
                        >
